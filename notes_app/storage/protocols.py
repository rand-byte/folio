...
from notes_app.enums import AttachmentRejectionReason, NotebookIcon
...
class NoteRepositoryProtocol(Protocol):
    ...
    (live search query, sort dropdown) happens in :mod:`notes_app.search`.
    """
...
class AttachmentStoreProtocol(Protocol):
    ...
    def add_for_note(self, note_id: str, source_path: Path) -> Attachment:
        ...
        * the source file's :meth:`pathlib.Path.stat` size exceeds
          :data:`notes_app.config.defaults.MAX_ATTACHMENT_BYTES`
          (``EXCEEDS_SIZE_LIMIT``) — checked before any bytes enter
          memory;
        * the file's MIME type is outside
          :class:`notes_app.enums.MimeKind` (``UNSUPPORTED_MIME_TYPE``);
...
class RendererProtocol(Protocol):
    ...
    def render_into(
        ...
        *,
        ...
        place. Raises :class:`notes_app.models.parse_error.ParseError`
