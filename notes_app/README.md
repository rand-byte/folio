...
- **Each `MainWindow` test builds its own `Gtk.Application` with a *unique* id** (`ui/test_main_window.py` `_make_test_application()` mints `org.notes_app.NotesApp.test{N}` from a counter). A registered `GApplication` exports an `org.gtk.Application` D-Bus object at a path derived from its id on the process-wide session-bus connection, and each window keeps its application alive; a shared id would make the second `register()` in the process fail with `An object is already exported … at /org/notes_app/NotesApp/test`. The export path is a pure function of the id, so a distinct id per build — not the `NON_UNIQUE` flag, which leaves the path unchanged, nor GC timing — is what keeps registrations from colliding.
