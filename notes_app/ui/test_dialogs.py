"""Tests for :mod:`notes_app.ui.dialogs`."""
...
class DefaultConfirmDialogPresenterTests(unittest.TestCase):
    ...
    def test_construction_does_not_raise_with_no_parent(self) -> None:
        # Calling the presenter records the dialog in GTK's pending
        # state; the test exits before the dialog is actually shown,
        # which is fine because no ``main()`` loop is running.
        captured: list[bool] = []
        ...
        # The result has not arrived yet (no main loop) — that is
        # the expected state for this test.
