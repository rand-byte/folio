...
| `toolbar.py` | 429 | Top `Gtk.HeaderBar` — *New* button (calls `make_initial_source(app_state.selection)` so the new note inherits the current tag selection's `:tags:` line), search entry whose `text` is **bound bidirectionally** to `AppState:query` (GObject's own echo-suppression replaces the old guard flag and removes the cursor-reset that reversed typed characters), an empty centre slot (no breadcrumb in the flat library), View/Source toggle (kept on explicit `notify::view-mode` handlers fenced by `_suppress_signal_writeback`, since the enum maps to two toggle buttons), a standalone note-scoped *Delete* trash-icon button next to *New* (sensitive only with a selection), and an app-scoped *Help* button (icon + `Syntax` label) that targets the application's `app.help` action via `set_action_name`. There are **no overflow menus** — the former note-scoped *More* (Duplicate/Delete) and app-scoped primary (hamburger) *Help* menus are gone: *Duplicate* was dropped and *Delete* / *Help* were promoted to direct buttons. The delete confirm reads the target note from the **store** (`get_note`), not the repository. |
