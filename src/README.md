...
| `giruntime/controllers` | `enums`, `models`, `config`, `search`, `storage.protocols`, `gi` (for `GObject`) | concrete `storage`, `giruntime/ui` |
...
| Add a note-level user action | `giruntime/controllers/note_controller.py` (mutate + emit signal) → caller in `giruntime/ui/toolbar.py` or `giruntime/ui/note_editor.py` | repository protocol if storage shape changes |
...
| Add a new sort key / smart filter | `enums.py` (`NoteSortKey` / `SmartFilter`, e.g. the existing `ALL` / `UNTAGGED`) → `search/note_filter.py` → `giruntime/ui/note_list.py` (dropdown) and / or `giruntime/ui/sidebar.py` (Library section row) | tests in `search/test_note_filter.py` |
| Change the note-list row title/snippet | the *derivation* in `asciidoc/summary.py` (`derive_summary`); the *presentation* in `giruntime/ui/note_list.py` `_make_note_row` + classes in `giruntime/ui/css/app.css` (`.note-title` / `.note-snippet` / `.note-meta`) | `storage/note_repository.py` only if the cached-column contract changes; a backfill migration if existing rows must be rewritten |
...
The `notes-changed` listener is `MainWindow`: it connects the controller's
signal to a handler that fans out to `NoteList.refresh()` + `Sidebar.refresh()`,
so a create / edit / delete propagates to the note list and the sidebar tag
section even when the filter and selection are unchanged (the panes otherwise
re-query only on `notify::selection` / `notify::selected-note-id` /
`notify::query` / sort changes). Both `refresh()` methods are idempotent and
repository-driven, so the panes stay unaware of the controller.
...
| `main_window.py` | 423 | `MainWindow` — the three-pane shell: sidebar │ note list │ `Gtk.Stack(view ↔ editor)`. Toolbar is set as the title bar. The initial window width is derived from the rendered article column (`_default_window_width` + `NoteView.preferred_column_width_px()`) so the fixed-width column opens fully visible. No notebook plumbing — the window is wired with the single note repository. Owns **two** signal subscriptions: `AppState:notify::view-mode` (flush editor + refresh view, then swap the stack) and `NoteController::notes-changed` (fan out to `NoteList.refresh()` + `Sidebar.refresh()` so mutations propagate without a manual reload). |
