...
| Change what can be **escaped** | usually nothing: `asciidoc/inline_parser.py`'s `_try_consume_construct` is the recogniser *and* the escape test (`_recognise_escaped_construct` runs it one position along), so a construct added there becomes escapable by construction. Touch `_try_consume_escape` only to change the *rule* — which backslash is consumed, or how much source the escape makes literal. The four deliberate divergences from Asciidoctor live in that module's docstring and in `test_inline_parser.py`'s `EscapeDivergesFromTheReferenceTests`; a comparison against the reference that "finds a bug" in one of them is finding a decision | `giruntime/ui/language_spec.lang` (the `escape` context, dispatched ahead of every inline context) + its `test_language_spec.py` fixtures; `system_docs/help.adoc` (*Escaping*); a re-derive `Migration`, because an escape changes the *characters* a summary flattens to |
...
| Change block tint insets / wash shape | `giruntime/ui/note_render/tag_table.py` (`build_wash_specs` — the inset/shape half; the tint comes from the palette) | `test_tag_table.py`, `note_render/test_article_text_view.py` wash-rect tests (`ArticleTextViewLayoutValidationTests` guards that wash geometry is measured against a *validated* layout — it must not settle a main loop before snapshotting, or it passes against the bug it exists to catch) |
...
| Change **any vertical gap** in the rendered view | `giruntime/ui/note_render/tag_table.py` — every vertical gap is a ratio of the measured body line height (`*_LINES`), resolved by `_gap_px` at build time, because the rhythm they are judged against is itself font-relative: one blank line between blocks is one line height. Write a new ratio as `N / REFERENCE_LINE_HEIGHT_PX` so the pixel value it was authored at stays visible. Horizontal insets are unrelated and still derive from the M-width. Body headings are the one place a gap is *not* an independent ratio: only the below gap is, and the above gap is a whole multiple of the resolved result, so the 2 : 1 model survives integer rounding at every font size | `test_tag_table.py` `VerticalGapsScaleWithTheFontTests` (identity at the reference, scaling away from it). Also check it **by eye at more than one font size** — a gap that looks right at one size is exactly the bug this structure removes, and the unit tests cannot see it. `dev-environment.md` has the Xvfb recipe; set `gtk-font-name` in a throwaway `$XDG_CONFIG_HOME/gtk-4.0/settings.ini` to vary the font, and size the virtual screen generously — there is no window manager, so anything past the window's edge is silently cropped, and the article column is a fixed 66 characters so the width it needs grows with the font |
| Handle a **font change** at runtime (or add a font preference) | `giruntime/ui/note_render/tag_table.py` `apply_metrics` rewrites every font-derived value on a live table — the geometry twin of `apply_palette`, and in place for the same reason: a `Gtk.TextBuffer` is bound to its tag table for life. `giruntime/ui/article_container.py` `invalidate_font_metrics` drops the cached measurements and reports whether they moved, so a colour-only `do_css_changed` (it fires on hover and focus) skips the expensive path. **Neither is sufficient alone**: table column widths and image scaling are computed during `render_into` from the injected measurers rather than stored on tags, so a metric change also needs a re-render, plus a recomputed column/window width and a scroll position restored by `Gtk.TextMark` anchor rather than pixel offset — that wiring is not yet in place, so today a font change mid-session still leaves stale geometry | `test_tag_table.py` `ApplyMetricsMatchesAFreshBuildTests` (re-measure ≡ fresh build, both directions); `test_article_container.py` `ArticleContainerFontMetricInvalidationTests` |
| Change how **monospace** looks or is measured | `giruntime/ui/note_render/tag_table.py` `MONOSPACE_FAMILY` / `MONOSPACE_SCALE` — public because they are a contract, not a local choice. The `MONOSPACE` tag draws with them and `note_view.make_cell_width_measurer` measures table cells with the same two attributes; a local copy in either place sizes a column for text a different size from what lands in it. The scale is an optical correction (a monospace face renders larger than the proportional body face at equal nominal size). A heading outranks the `MONOSPACE` tag, so inline code in a heading takes the heading's size — GTK gives a property to the highest-priority tag rather than combining them | `test_tag_table.py` `MonospaceSizeCorrectionTests`; `test_note_view.py` `CellWidthMeasurerAppliesTheMonospaceCorrectionTests`. Also check by eye that box-drawing characters in a code block still connect and that a monospace table column still aligns — neither is visible to a unit test |
...
| Change the spacing around a **subordinate line** (the metadata line under the title, an admonition's kind label above its body) | `giruntime/ui/note_render/tag_table.py` — both pairs follow one rule: the gap *between* the two lines is non-zero (or a descender in the line above closes it) and *smaller* than the gap bounding the unit (the metadata line's rule below it, the admonition card's outer padding), so the pair reads as one thing. The admonition's inner gap is contributed once, by the label's `pixels-below-lines`: `pixels-above-lines` applies to every logical line, so putting it on the body would space the body's own lines apart too | `test_tag_table.py` `SubordinateLineGapTests` — ordering assertions, not values, because the literals are a visual judgement expected to be re-tuned |
...
- `schema_version(version PK)` records applied migrations. Latest is **v6** (a second re-derive of `title` / `snippet` / `note_tags`, this time because backslash escaping changed the characters a summary flattens to; it delegates to v5's body rather than restating it, and like v5 leaves timestamps untouched).
