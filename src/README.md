...
- **`note_list.py`** — middle pane: a `ListView` over `SingleSelection(SortListModel(FilterListModel(NoteListStore)))`, reusing the `search.note_filter` predicates. Selection is one source of truth (`AppState`). Carries the focus-local `Delete` shortcut (a `Gtk.ShortcutController` at `LOCAL` scope activating `win.delete-note`), so `Delete` deletes the selected note only while this pane has focus, never inside the editor.
