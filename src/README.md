...
| Tune article column margins | `config/defaults.py` (the three `ARTICLE_*` multipliers) | none — `giruntime/ui/note_view.py` reads the constants once at `NoteView.__init__` and applies them to the inner `Gtk.TextView`'s four margins |
