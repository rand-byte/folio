...
- **`inline_parser.py`** — `parse_inline(line, line_no) -> tuple[InlineNode, ...]`. **Strict**: unpaired markers raise. Nesting is bounded by `MAX_INLINE_DEPTH` (one Python frame per level), so a pathological line raises `ParseError` rather than `RecursionError`.
