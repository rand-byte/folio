...
| `enums.py` | 297 | **Single home** for every categorical constant. Persisted enums use `StrEnum` with stable values; transient ones use `auto()`. `MimeKind` is gone (attachments carry no classification), and `AttachmentRejectionReason` lost its `UNSUPPORTED_MIME_TYPE` member with it. `SystemDocument` (value = the package-relative filename of each bundled system document) and `HelpSection` (value = the exact `==` heading text of each help bucket) key the `system_docs` loader and the help window's navigation respectively. `WashShape` (`FILL` / `HAIRLINE` / `LEFT_BAR`) is the rendered-view wash painter's shape dispatch — see `tag_table.WashSpec`. |
