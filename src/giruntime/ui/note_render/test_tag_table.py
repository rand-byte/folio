...
class BuildSheetWashTests(unittest.TestCase):
    """:func:`build_sheet_wash` owns the note sheet colour.

    The sheet is not keyed to a paragraph tag, so it lives outside
    :func:`build_wash_specs`; these tests pin its colour contract the
    same way the wash-spec tests pin the paragraph washes.
    """
    ...
    def test_tint_is_a_four_channel_value(self) -> None:
        # The tint is an RGBA 4-tuple in [0, 1]; the painter feeds it
        # straight to Gdk.RGBA, so a wrong arity would fail at paint time.
        self.assertEqual(len(self.wash.tint), 4)
        for channel in self.wash.tint:
    ...
    def test_sheet_is_opaque(self) -> None:
        ...
        # behind the text.
        self.assertEqual(self.wash.tint[3], 1.0)
